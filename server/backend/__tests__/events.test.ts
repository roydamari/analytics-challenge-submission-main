...
describe("main test", () => {
  ...
  it("can sort events by date", async () => {
    const { body: events } = await request(app)
      ...
      .expect(200);
    const { body: events2 } = await request(app)
      .get("/events/all-filtered")
      .query({
        offset: 5,
        sorting: "+date",
  ...
  it("can post new event", async () => {
    await request(app)
      .post("/events")
      .send(mockData.events[0])
      .expect(({ status }) => {
        if (status !== 200 && status !== 201)
          throw new Error(
            `Expected Status to be '200 OK' Or '201 Created', instead received ${status}`
          );
      });
